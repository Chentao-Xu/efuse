...
#ifdef CONFIG_COMPAT
    if (is_compat && (ptr_to_compat(dst[i].iov_base) != fiov[i].base ||
...
long fuse_do_ioctl(struct file *file, unsigned int cmd, unsigned long arg,
  ...
  struct fuse_ioctl_in inarg = {
      .fh = ff->fh, .cmd = cmd, .arg = arg, .flags = flags};
  struct fuse_ioctl_out outarg;
...
retry:
  ...
  if (outarg.flags & FUSE_IOCTL_RETRY) {
     ...
     */
    err = -ENOMEM;
    if (in_iovs > FUSE_IOCTL_MAX_IOV || out_iovs > FUSE_IOCTL_MAX_IOV ||
        in_iovs + out_iovs > FUSE_IOCTL_MAX_IOV)
      goto out;

    vaddr = kmap_local_page(ap.pages[0]);
    err = fuse_copy_ioctl_iovec(fm->fc, iov_page, vaddr, transferred,
                                in_iovs + out_iovs,
...
long fuse_ioctl_common(struct file *file, unsigned int cmd, unsigned long arg,
                       unsigned int flags) {
...
int fuse_fileattr_set(struct mnt_idmap *idmap, struct dentry *dentry,
                      struct fileattr *fa) {
  struct inode *inode = d_inode(dentry);
  struct fuse_file *ff;
  ...
  if (fa->flags_valid) {
    err = fuse_priv_ioctl(inode, ff, FS_IOC_SETFLAGS, &flags, sizeof(flags));
    if (err)
      goto cleanup;
  } else {
    ...
    err = fuse_priv_ioctl(inode, ff, FS_IOC_FSSETXATTR, &xfa, sizeof(xfa));
  }
