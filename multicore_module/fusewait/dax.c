...
static void __kick_dmap_free_worker(struct fuse_conn_dax *fcd,
  ...
  free_threshold =
      max_t(unsigned long, fcd->nr_ranges *FUSE_DAX_RECLAIM_THRESHOLD / 100, 1);
...
static struct fuse_dax_mapping *alloc_dax_mapping(struct fuse_conn_dax *fcd) {
  ...
  spin_lock(&fcd->lock);
  dmap = list_first_entry_or_null(&fcd->free_ranges, struct fuse_dax_mapping,
                                  list);
  if (dmap) {
...
static int dmap_removemapping_list(struct inode *inode, unsigned int num,
  ...
  list_for_each_entry(dmap, to_remove, list) {
    ...
    if (i >= nr_alloc || num == 0) {
      memset(&inarg, 0, sizeof(inarg));
      inarg.count = i;
      ret = fuse_send_removemapping(inode, &inarg, remove_one);
      if (ret)
        goto out;
      ptr = remove_one;
      i = 0;
...
static void dmap_reinit_add_to_free_pool(struct fuse_conn_dax *fcd,
  ...
  pr_debug("fuse: freeing memory range start_idx=0x%lx end_idx=0x%lx "
           ...
           dmap->itn.start, dmap->itn.last, dmap->window_offset, dmap->length);
  __dmap_remove_busy_list(fcd, dmap);
  dmap->inode = NULL;
...
static void inode_reclaim_dmap_range(struct fuse_conn_dax *fcd,
  ...
  while (1) {
    node = interval_tree_iter_first(&fi->dax->tree, start_idx, end_idx);
    if (!node)
      break;
...
static int fuse_upgrade_dax_mapping(struct inode *inode, loff_t pos,
  ...
  ret = fuse_setup_one_mapping(inode, pos >> FUSE_DAX_SHIFT, dmap, true, true);
  if (ret < 0)
    goto out_err;
out_fill_iomap:
  fuse_fill_iomap(inode, pos, length, iomap, dmap, flags);
  ...
  if (node) {
    ...
    if (writable && !dmap->writable) {
      ...
      pr_debug("%s: Upgrading mapping at offset 0x%llx length 0x%llx\n",
               __func__, pos, length);
      return fuse_upgrade_dax_mapping(inode, pos, length, flags, iomap);
    } else {
      fuse_fill_iomap(inode, pos, length, iomap, dmap, flags);
      up_read(&fi->dax->sem);
      return 0;
    ...
    return fuse_setup_new_dax_mapping(inode, pos, length, flags, iomap);
  }
...
iomap_hole:
  fuse_fill_iomap_hole(iomap, length);
  pr_debug("%s returning hole mapping. pos=0x%llx length_asked=0x%llx "
           "length_returned=0x%llx\n",
...
static int __fuse_dax_break_layouts(struct inode *inode, bool *retry,
  ...
  *retry = true;
  return ___wait_var_event(&page->_refcount, atomic_read(&page->_refcount) == 1,
                           TASK_INTERRUPTIBLE, 0, 0, fuse_wait_dax_page(inode));
}
...
int fuse_dax_break_layouts(struct inode *inode, u64 dmap_start, u64 dmap_end) {
  ...
  do {
    retry = false;
    ret = __fuse_dax_break_layouts(inode, &retry, dmap_start, dmap_end);
  } while (ret == 0 && retry);
...
static ssize_t fuse_dax_direct_write(struct kiocb *iocb,
...
static vm_fault_t fuse_dax_fault(struct vm_fault *vmf) {
  return __fuse_dax_fault(vmf, PE_SIZE_PTE, vmf->flags & FAULT_FLAG_WRITE);
}
...
static int dmap_writeback_invalidate(struct inode *inode,
  ...
  ret = invalidate_inode_pages2_range(inode->i_mapping, start_pos >> PAGE_SHIFT,
                                      end_pos >> PAGE_SHIFT);
  if (ret)
...
inode_inline_reclaim_one_dmap(struct fuse_conn_dax *fcd, struct inode *inode,
  ...
  pr_debug("fuse: %s: inline reclaimed memory range. inode=%p, "
           "window_offset=0x%llx, length=0x%llx\n",
...
static int lookup_and_reclaim_dmap(struct fuse_conn_dax *fcd,
                                   struct inode *inode, unsigned long start_idx,
...
static int try_to_free_dmap_chunks(struct fuse_conn_dax *fcd,
  ...
  while (1) {
    ...
    list_for_each_entry_safe(pos, temp, &fcd->busy_ranges, busy_list) {
...
static void fuse_dax_free_mem_worker(struct work_struct *work) {
  ...
  struct fuse_conn_dax *fcd =
      container_of(work, struct fuse_conn_dax, free_work.work);
...
static int fuse_dax_mem_range_init(struct fuse_conn_dax *fcd) {
  ...
  id = dax_read_lock();
  nr_pages = dax_direct_access(fcd->dev, 0, PHYS_PFN(dax_size), DAX_ACCESS,
                               NULL, NULL);
  dax_read_unlock(id);
  if (nr_pages < 0) {
