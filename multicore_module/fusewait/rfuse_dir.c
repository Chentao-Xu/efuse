...
int rfuse_do_setattr(struct dentry *dentry, struct iattr *attr, struct file *file){
	...
	fuse_change_attributes_common(inode, &outarg->attr, rfuse_attr_timeout(outarg),fuse_get_cache_mask(inode)); 
...
int rfuse_mkdir(struct mnt_idmap *idmap, struct inode *dir,//struct user_namespace *mnt_userns
...
int rfuse_symlink(struct mnt_idmap *idmap, struct inode *dir,//struct user_namespace *mnt_userns
