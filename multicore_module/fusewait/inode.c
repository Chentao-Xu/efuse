...
int fuse_reverse_inval_inode(struct fuse_conn *fc, u64 nodeid, loff_t offset,
                             loff_t len) {
  struct fuse_inode *fi;
  struct inode *inode;
  pgoff_t pg_start;
...
// static void convert_fuse_statfs(struct kstatfs *stbuf,
...
static int fuse_parse_param(struct fs_context *fsc,
...
static struct dentry *fuse_fh_to_dentry(struct super_block *sb, struct fid *fid,
...
static struct dentry *fuse_fh_to_parent(struct super_block *sb, struct fid *fid,
                                        int fh_len, int fh_type) {
...
// static void process_init_limits(struct fuse_conn *fc,
...
//     if (!cap_sys_admin && fc->congestion_threshold > max_user_congthresh)
...
//         fc->max_pages = min_t(unsigned int, fc->max_pages_limit,
...
//           FUSE_SETXATTR_EXT | FUSE_INIT_EXT | FUSE_SECURITY_CTX |
...
static int fuse_bdi_init(struct fuse_conn *fc, struct super_block *sb) {
  ...
  err = super_setup_bdi_name(sb, "%u:%u%s", MAJOR(fc->dev), MINOR(fc->dev),
                             suffix);
  if (err)
    return err;
...
static int fuse_fill_super(struct super_block *sb, struct fs_context *fsc) {
  ...
  if (!ctx->file || !ctx->rootmode_present || !ctx->user_id_present ||
      !ctx->group_id_present)
    return -EINVAL;
...
#else
static inline int register_fuseblk(void) { return 0; }
...
static int __init fuse_fs_init(void) {
  ...
  fuse_inode_cachep = kmem_cache_create(
      "fuse_inode", sizeof(struct fuse_inode), 0,
