...
MODULE_PARM_DESC(allow_sys_admin_access,
                 "Allow users with CAP_SYS_ADMIN in initial userns to bypass "
                 "allow_other access check");
...
int fuse_valid_type(int m) {
  return S_ISREG(m) || S_ISDIR(m) || S_ISLNK(m) || S_ISCHR(m) || S_ISBLK(m) ||
...
int fuse_lookup_name(struct super_block *sb, u64 nodeid,
  ...
  *inode = fuse_iget(sb, outarg->nodeid, outarg->generation, &outarg->attr,
                     entry_attr_timeout(outarg), attr_version);
  err = -ENOMEM;
  if (!*inode) {
    fuse_queue_forget(fm->fc, forget, outarg->nodeid, 1);
    goto out;
...
static int get_security_context(struct dentry *entry, umode_t mode,
  ...
  err = security_dentry_init_security(entry, mode, &entry->d_name, &name, &ctx,
                                      &ctxlen);
  if (err) {
    if (err != -EOPNOTSUPP)
      goto out_err;
...
static int get_create_ext(struct fuse_args *args, struct inode *dir,
...
static int create_new_entry(struct fuse_mount *fm, struct fuse_args *args,
  ...
  inode = fuse_iget(dir->i_sb, outarg.nodeid, outarg.generation, &outarg.attr,
...
static int fuse_rename2(struct mnt_idmap *idmap, struct inode *olddir,
  ...
  if (flags) {
    ...
    err = rfuse_rename_common(olddir, oldent, newdir, newent, 0, FUSE_RENAME,
...
static int fuse_do_getattr(struct inode *inode, struct kstat *stat,
  ...
  //     err = -EIO;
  //   } else {
  //     fuse_change_attributes(inode, &outarg.attr, attr_timeout(&outarg),
  //                            attr_version);
  //     if (stat)
...
static inline bool fuse_permissible_uidgid(struct fuse_conn *fc) {
  ...
  return (uid_eq(cred->euid, fc->user_id) && uid_eq(cred->suid, fc->user_id) &&
          uid_eq(cred->uid, fc->user_id) && gid_eq(cred->egid, fc->group_id) &&
...
static long fuse_dir_compat_ioctl(struct file *file, unsigned int cmd,
  ...
  return fuse_ioctl_common(file, cmd, arg, FUSE_IOCTL_COMPAT | FUSE_IOCTL_DIR);
}
...
int fuse_do_setattr(struct dentry *dentry, struct iattr *attr,
  ...
  //   fuse_change_attributes_common(inode, &outarg.attr, attr_timeout(&outarg),
...
static int fuse_setattr(struct mnt_idmap *idmap, struct dentry *entry,
  ...
  if (attr->ia_valid & (ATTR_KILL_SUID | ATTR_KILL_SGID)) {
    attr->ia_valid &= ~(ATTR_KILL_SUID | ATTR_KILL_SGID | ATTR_MODE);
...
static int fuse_getattr(struct mnt_idmap *idmap, const struct path *path,
                        struct kstat *stat, u32 request_mask,
                        unsigned int flags) {
