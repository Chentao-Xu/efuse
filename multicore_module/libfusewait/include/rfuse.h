 ...
 * 'Checks' the ring buffer's 'tail'... Such sequence should accompany a 'memory
 * barrier' Okay, so I originally wanted to add it by my taste. But, it was
 * almost impossible to check whether I have implemented correctly. So, I just
 * followed the basic sequence of io_uring using the bellow MACROs in user space
 * would provide memory barriers (at least)equal to io_uring, which is a well
 * known shared memory ring buffer.
 */

#ifdef __cplusplus
#include <atomic>

template <typename T> static inline void RFUSE_WRITE_ONCE(T &var, T val) {
  std::atomic_store_explicit(reinterpret_cast<std::atomic<T> *>(&var), val,
                             std::memory_order_relaxed);
}
template <typename T> static inline T RFUSE_READ_ONCE(const T &var) {
  return std::atomic_load_explicit(
      reinterpret_cast<const std::atomic<T> *>(&var),
      std::memory_order_relaxed);
}

template <typename T> static inline void rfuse_smp_store_release(T *p, T v) {
  std::atomic_store_explicit(reinterpret_cast<std::atomic<T> *>(p), v,
                             std::memory_order_release);
}

template <typename T> static inline T rfuse_smp_load_acquire(const T *p) {
...
#define RFUSE_WRITE_ONCE(var, val)                                             \
  atomic_store_explicit((_Atomic __typeof__(var) *)&(var), (val),              \
...
// state for recognizing whether the completion thread is woken up by user
// request
#define COMP_NEED_WAKEUP_FROM_USER (1U << 3)
#define COMP_ALL_COMP_WORKING (1U << 4)

#define RFUSE_NUM_IQUEUE 2
