...
static int send_reply(old_fuse_req_t req, int error, const void *arg,
					  size_t argsize)
{
	struct iovec iov[2];
	int count = 1;
...
int fuse_passthrough_open(old_fuse_req_t req, int fd)
{
...
int fuse_passthrough_close(old_fuse_req_t req, int backing_id)
{
	int ret;
...
static void do_tmpfile(old_fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	struct fuse_create_in *arg = (struct fuse_create_in *)inarg;

	if (req->se->op.old_tmpfile)
	{
		struct fuse_file_info fi;
		...
		req->se->op.old_tmpfile(req, nodeid, arg->mode, &fi);
	}
	else
		old_fuse_reply_err(req, ENOSYS);
}
...
static __attribute__((no_sanitize("thread"))) void do_init(old_fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
...
void old_fuse_req_interrupt_func1(old_fuse_req_t req, old_fuse_interrupt_func_t func,
								 void *data)
{
						   ...
						   struct libfuse_version *version, void *userdata)
{
	...
	se->bufsize = FUSE_MAX_PAGES * getpagesize() +
