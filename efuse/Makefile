...
LIBBPF_OBJ := $(abspath $(OUTPUT)/libbpf.so)
...
PATCH_FROM_HELPER_H := ./patch/bpf_helper_defs.h
PATCH_TO_HELPER_H := $(OUTPUT)/bpf/bpf_helper_defs.h
...
$(OUTPUT)/libextfuse.so: $(OUTPUT)/ebpf.o $(LIBBPF_OBJ) | $(OUTPUT)
	$(call msg,HOSTLD,$@)
	$(Q)$(CC) $(CFLAGS) -o $@ $^ $(ALL_LDFLAGS) -lelf -lz -shared

$(OUTPUT)/ebpf.o: ebpf.c $(wildcard *.h) $(OUTPUT)/extfuse.skel.h | $(OUTPUT)
	$(call msg,CC,$@)
	$(Q)$(CC) $(CFLAGS) -fPIC $(INCLUDES) -c $< -o $@
...
$(LIBBPF_OBJ): $(wildcard $(LIBBPF_SRC)/*.[ch] $(LIBBPF_SRC)/Makefile) | $(OUTPUT)/libbpf
	...
	$(Q)$(MAKE) -C $(LIBBPF_SRC)		      \
		    ...
		    install
	$(Q)cp $(PATCH_FROM_HELPER_H) $(PATCH_TO_HELPER_H)
