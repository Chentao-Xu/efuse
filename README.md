...
| **目标3：FUSE I/O 请求的特殊优化** | 全部完成 <br/>100%| 1. 支持直通路径：eBPF 直接读取文件内容。 <br />2. 支持缓存路径：将内容存入 eBPF map 缓存。 <br />3. 设计请求调度策略实现直通与缓存路径选择。<br />4. 读写性能提升 3~5 倍。 |
...
✅ 调研现有技术栈以及相关优秀开源项目。\
✅ 在虚拟机中搭建测试开发环境。\
✅ 深度学习eBPF开发流程和FUSE工作流程。\
✅ 在内核指定位置添加eFuse相关eBPF程序挂载点。\
✅ 在内核中实现并注册eBPF helper函数。\
✅ 初步实现FUSE元数据请求的绕过逻辑，用户态文件系统协调配合。\
✅ 实现FUSE I/O请求的绕过逻辑，包含map缓存和直通两条路径。\
✅ 设计并实现FUSE I/O请求两条路径的智能调度算法。\
✅ 设计为每个核心分配一个环形管道的优化结构。\
✅ 初步实现多核优化模块，初赛时未集成到主项目。\
✅ 进行初步的性能评估，在虚拟机中进行测试
...
✅ 完成多核优化模块和主项目的结构统一，实现集成。\
✅ 实现请求队列的负载监控和均衡调度。\
✅ 分别尝试使用eBPF和直接在内核中完成负载监控。\
✅ 仿真实现支持内部计算能力的 CSD 存储设备。\
✅ 实现I/O堆栈层面的设备合集和内核路径调度。\
✅ 进一步丰富性能测试场景。\
✅ 部署等价的实体机测试环境，进行测试。

- 对初赛内容的改进

✅ 对部分FUSE请求的绕过逻辑做细节优化，进一步略微提升性能。\
✅ 设计更多、更为合理的性能测试场景。\
✅ 对libfuse库和内核部分文件进行了适配多核优化模块的重构。
...
在原始 FUSE 架构中，在应用程序发出系统调用后，首先由内核中的 VFS (Virtual File System) 捕获，并发送至 FUSE 内核驱动，将其放入待处理队列中。队列中的请求依次提交给用户态文件系统处理，将处理完的结果返回内核，最终交付给应用程序。
...
<img src="./images/18.大结构图new.png" width="70%">
...
当检测到当前环形通道拥塞时，efuse_get_iqueue_for_async让**异步请求**会被​​轮询分配到其他空闲通道。

若轮询后仍拥塞，进行随机优化，EFUSE随机选择其他通道（最多尝试10次）。
...
<img src="./images/决赛测试结果图4.png" width="60%" height="60%">
</div>
...
| 参数指标 | __测试1__  | __测试2__ | 
| -------- | -------- | ----| 
| __模拟场景__  | 单文件随机读写 | 小文件随机读写 |
| __读写比例__  | 7:3 | 7:3 | 
| __访问模式__  | 随机读写 | 随机读写 | 
| __读写块大小__  | 4K | 4K | 
| __读写总大小__  | 8M | 100M |
| __文件数/大小__  | 单文件 | 50个文件，每个512K | 
| __任务数__  | 8 | 16 | 
| __队列深度__  | 256 | 256 | 
| __运行时间__  | 60s | 60s | 
...
<img src="./images/决赛测试结果图8-2.png" width="60%" height="60%">
</div>
...
- **测试1**和 **测试2** 的读写单位都为小数据块（4K），IOPS 、读写吞吐量、读写操作的处理时延都能够实现远超原版 FUSE 的性能，提升约 4~5 倍，充分发挥了 **eBPF map 缓存路径** 和 **CSD 存算一体设备路径** 的优势。

- 对于读写单位为大数据块的测试场景，原本 FUSE 的吞吐量本身达到了较高水平，而 eFuse 一方面无法一次存入大数据块至 eBPF map 中，另一方面需要在 eBPF 程序中进行多次数据拼接，总体性能的优化效果略低于小数据块读写场景。优化在大数据块场景下的读写性能将成为 eFuse 在之后的优化方向和挑战。
