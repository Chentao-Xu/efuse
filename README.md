...
| **校内导师** | 夏文、李诗逸 |
  ...
  为此，我们提出eFuse，一个结合eBPF技术的FUSE性能优化方案。其中包括**通过eBPF map实现文件元数据和文件内容在内核中的缓存**，**在内核eBPF程序中对FUSE请求快速处理**，**设计更为合理的内核请求队列结构**，**优化请求处理和调度管理**。提升各个负载场景下FUSE的性能和可扩展性，**初赛阶段性能提升约3倍**。上述设计不改变原本FUSE的架构和接口标准，能够实现对现有FUSE的完全兼容。
